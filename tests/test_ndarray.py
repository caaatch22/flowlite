...
from flowlite import backend_ndarray as nd
...
def test_reduce_sum(params, device):
    ...
    np.testing.assert_allclose(
        _A.sum(axis=axis, keepdims=True), A.sum(dim=axis, keepdim=True).numpy(), atol=1e-5, rtol=1e-5
    )
...
def test_reduce_max(params, device):
    ...
    A = nd.array(_A, device=device)
    np.testing.assert_allclose(
        _A.max(axis=axis, keepdims=True), A.max(dim=axis, keepdim=True).numpy(), atol=1e-5, rtol=1e-5
    )
...
def test_matmul(m, n, p, device):
    ...
    A = nd.array(_A, device=device)
    B = nd.array(_B, device=device)
    np.testing.assert_allclose((A @ B).numpy(), _A @ _B, rtol=1e-4, atol=1e-4)
